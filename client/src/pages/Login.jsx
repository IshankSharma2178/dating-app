...
export default function Login() {
    ...
    useEffect(() => {
        if (gender) {
            ...
            fetch(`/api/partners/${gender}`)
                ...
                .then((data) => {
                    if (data.partners) {
                        const merged = [...local.map(p => ({ name: p.name, email: p.email }))];
    ...
    useEffect(() => {
        setFiltered(partners.filter((p) => p.name.toLowerCase().includes(search.toLowerCase())));
    }, [search, partners]);
    ...
    const handleSendOtp = async () => {
        ...
        let targetEmail = '';
        if (mode === 'select') {
            if (!selectedName) { setError('Please select your name from the list'); return; }
            if (!selectedEmail) { setError('Email not found for this name'); return; }
                ...
                {mode === 'otp' ? (
                    <form onSubmit={handleVerifyOtp}>
                        <p style={{ fontSize: '13px', color: '#666', marginBottom: '12px' }}>
                            An OTP was sent to <strong>{email || selectedEmail}</strong>
                        </p>
                        ...
                        <p style={{ marginTop: '10px', fontSize: '13px' }}>
                            <button type="button" onClick={() => { setMode('select'); setOtp(''); setError(''); setMessage(''); setEmail(''); setSelectedName(''); setSelectedEmail(''); }} className="link-btn" style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'underline' }}>
                                Change email / name
                            </button>
                        </p>
                    </form>
                    ...
                    <>
                        ...
                        {mode === 'select' && (
                            <div className="form-group" ref={dropdownRef}>
                                    ...
                                    {isOpen && (
                                        <div className="dropdown-list open">
                                            {filtered.length === 0 ? (
                                                ...
                                                filtered.map((p) => (
                                                    <div key={p.name} className={`dropdown-item ${selectedName === p.name ? 'selected' : ''}`} onClick={() => selectName(p.name, p.email)}>
                                                        {p.name}
                                                    </div>
                                                ))
                                            )}
                                ...
                                {selectedName && (
                                    <p className="field-hint" style={{ color: '#e74c3c', fontWeight: 600 }}>Selected: {selectedName} ({selectedEmail})</p>
                                )}
                                <button type="button" className="submit-btn" disabled={loading || !selectedName} onClick={handleSendOtp} style={{ marginTop: '12px' }}>
                        ...
                        {mode === 'email' && (
                            <div className="form-group">
                                ...
                                <p style={{ fontSize: '12px', color: '#999', marginTop: '4px' }}>
                                    If your name is in our records, use "Select my name" above instead.
                                </p>
                                <button type="button" className="submit-btn" disabled={loading || !email} onClick={handleSendOtp} style={{ marginTop: '8px' }}>
                                    {loading ? 'Sending...' : 'Send OTP'}
                                </button>
                            </div>
                        )}
