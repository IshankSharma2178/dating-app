...
export default function DateForm({ embedded = false }) {
  ...
  useEffect(() => {
    setFiltered(
      names.filter((n) =>
        n.toLowerCase().includes(partnerSearch.toLowerCase()),
      ),
    );
  }, [partnerSearch, names]);
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (!e.target.closest(".search-dropdown")) setIsOpen(false);
    };
  ...
  useEffect(() => {
    getPartnerStatus(oppositeGender)
      .then((partners) => {
        const statusMap = {};
        ...
        const sorted = [...allNames].sort(
          (a, b) => (statusMap[b] ? 1 : 0) - (statusMap[a] ? 1 : 0),
        );
        setNames(sorted);
      })
      .catch(() => {});
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!partnerName) {
      setError("Please select a partner name");
      return;
    }
    setError("");
    setSuccess("");
  ...
  const subtitle = "Lets, create a day you will never forget! ✨";
  const btnText =
    gender === "boys" ? "Send to My Girl 💌" : "Send to My Boy 💌";

  return (
    <div className="container">
      <div style={{ fontSize: "60px", lineHeight: 1, marginTop: "20px" }}>
        💖
      </div>
      <h1>Unlock, who already likes you! 🥰</h1>
      ...
      <form onSubmit={handleSubmit} className="date-form">
        ...
        <div className="form-group">
          ...
          <select name="food" value={form.food} onChange={handleChange}>
            ...
            <option value="Fast Food / Pizza">
              🍕 Pizza / Burgers & Fries
            </option>
            <option value="Fine Dining">🕯️ Fancy Candlelight Dinner</option>
            <option value="Street Food Hop">
              🍿 Street Food & Chaotic Walking
            </option>
            <option value="Surprise Me">
              🎁 Keep it a secret, surprise me!
            </option>
          </select>
        </div>
        ...
        <div className="form-group">
          ...
          <input
            ...
            value={form.location}
            onChange={handleChange}
            placeholder="e.g., Cozy cafe, Sushi, Surprise me!"
          />
        </div>
          ...
          <div className="radio-group">
            <label>
              <input
                ...
                checked={form.matchOutfits === "Yes, absolutely"}
                onChange={handleChange}
              />{" "}
              Yes, obviously! Twin with me.
            </label>
            <label>
              <input
                type="radio"
                name="matchOutfits"
                value="No, let us surprise each other"
                checked={form.matchOutfits === "No, let us surprise each other"}
                onChange={handleChange}
              />{" "}
              No, let's surprise each other.
            </label>
          </div>
        </div>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            onChange={handleChange}
            rows="3"
            placeholder="Favorite snacks, song requests, or demands..."
          />
        </div>
